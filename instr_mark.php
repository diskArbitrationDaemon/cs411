...
if (empty($stage)){
    ...
    if(mysql_num_rows($result)){
        ...
        //get questions
        $query = "SELECT QuestionName, Questions.QuestionID FROM Questions, Assignment, Course, Takes WHERE
        ...
        Questions.AssnID = Assignment.AssnID"; 
        ...
        if (mysql_num_rows($result) == 0){
            ...
            while ($row = mysql_fetch_array($result)){
                ...
                $query = "SELECT Mark FROM Results WHERE StudentID='".$_GET['StudentID']."' AND QuestionID='".$row['QuestionID']."'";
                $markQueryResult = mysql_query($query);
                $mark = "";
                if (mysql_num_rows($markQueryResult)){
                    $markRow = mysql_fetch_array($markQueryResult);
                    $mark = $markRow['Mark'];
                }
                print("<tr><td >" . $row['QuestionName'] . "</td><td><input type=text name=Q". $row['QuestionID'] ." length=5 value=\"".$mark."\"></td></tr>\n");
